...
export function addMockData(store: AppStore) {
  ...
  store.dispatch(armyDataAction);

  store.dispatch({
    type: 'army/addUnit',
    payload: armyDataAction.payload.armyBookData.units.find(x => x.id === "lmZtl0E")
  })

  const selectionId = store.getState().list.units[0].id;
...
const armyDataAction = {
  ...
  meta: {
    arg: {
      armyUid: 'w7qor7b2kuifcyvk',
      gameSystem: 'gf',
      reset: false
    },
    requestId: 'izL1_BdZ-DEuSRrm5jLal',
