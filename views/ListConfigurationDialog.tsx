...
export default function ListConfigurationDialog({
  isEdit,
  open,
  setOpen,
  customArmies,
  ...
  const factionRelation = army.childData?.filter((c) => c.factionRelation)[0]
    ?.factionRelation;
  ...
  const create = async () => {
    ...
    if (factionRelation) {
      const childArmy = army.childData.find(
        (child) => child.name === selectedChild
      );

      const childArmyData = await WebappApiService.getArmyBookData(
        childArmy.uid,
        army.gameSystem
      );

      dispatch(loadArmyData(childArmyData));

      finish({ ...army, ...childArmy, data: childArmyData });
    } else {
      finish(army);
    }
  };
  ...
  const close = () => {
    router.replace({ query: { ...router.query, armyId: null } }, null, {
      shallow: true,
    });
    setOpen(false);
  };
  ...
  const rootArmy =
    army.childData && army.childData.find((c) => c.name === army.data.name);
            ...
            {!isEdit && army.childData && (
              <>
                ...
                <List className="pt-0">
                  ...
                  {army.childData
                    .filter((c) => c.name !== army.data.name)
                    .map((child, index) => (
