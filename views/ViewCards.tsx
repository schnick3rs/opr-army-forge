...
export default function ViewCards(props: ViewCardsProps) {
  const { prefs, showPsychic } = props;
  ...
  const units = rootUnits.map((u) => makeCopy(u));
  const hasJoined = (unit: ISelectedUnit) =>
    list.units.some((u) => u.joinToUnit === unit.selectionId);
  const usedRules = [];
  ...
  const unitAsKey = (unit: ISelectedUnit) => {
    return {
      ...
      combined: hasJoined(unit) ? nanoid(5) : false,
    };
  };
  ...
  const getUnitCard = (unit: ISelectedUnit, unitCount: number) => {
    ...
    const units = [unit].concat(list.units.filter((u) => u.joinToUnit === unit.selectionId));
    return (
      <UnitCard
        rules={rules}
        units={units}
...
function UnitCard({ units, rules, count, prefs, ruleDefinitions }: UnitCardProps) {
  ...
  const getInternalForUnit = (unit: ISelectedUnit) => {
    const toughness = toughFromUnit(unit);
    return (
      <div key={unit.selectionId} className="card-body mb-4">
  ...
  return (
    <div className={style.card}>
      <Card elevation={1}>{units.map(u => (getInternalForUnit(u)))}</Card>
    </div>
  );
}
